...
export async function createProduct(formData:FormData){
  ...
  const {data:product,error:productError}=await supabase.from("products").insert({store_id:storeId,category_id:category.id,sku:productSku,name,description:v.specification||""}).select("id").single();
...
export async function updateProduct(formData:FormData){
  ...
  const {error:pError}=await supabase.from("products").update({name:v.name,sku:v.productSku,category_id:category.id,description:v.specification||"",updated_at:new Date().toISOString()}).eq("id",v.productId).eq("store_id",storeId);if(pError)fail("商品更新失敗："+pError.message);
