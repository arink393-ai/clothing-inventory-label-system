...
export default async function Members({ searchParams }: { searchParams: Promise<{ message?: string }> }) {
  ...
  const [{ data: customers }, { data: ledger }, { data: store }, { data: approvalData }] = await Promise.all([
  ...
  const approval = approvalData as { pin_configured?: boolean } | null;
  return <MemberManager rows={rows} history={history} message={message} canAdjust={["owner", "manager", "cashier"].includes(member!.role)} canDelete={["owner", "manager"].includes(member!.role)} adjustmentNeedsPin={member!.role === "cashier"} pinConfigured={approval?.pin_configured === true} spendAmount={Number(store?.points_spend_amount || 100)} pointValue={Number(store?.point_value || 1)} enabled={store?.points_enabled !== false}/>;
}
