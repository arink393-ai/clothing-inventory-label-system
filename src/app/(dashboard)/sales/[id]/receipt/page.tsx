...
export default async function Receipt({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: sale } = await supabase.from("sales").select("id,document_no,completed_at,subtotal,discount,total,payment_method,points_earned,points_redeemed,points_discount,transfer_account_last4,payment_note,payment_confirmed_by,customers(name,phone),sale_items(id,quantity,unit_price,product_variants(sku,color,size,products(name)))").eq("id", id).eq("store_id", member!.store_id).eq("status", "completed").maybeSingle();
  if (!sale) notFound();
