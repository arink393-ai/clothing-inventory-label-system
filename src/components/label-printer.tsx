...
export function LabelPrinter({ products, storeName }: { products: LabelProduct[]; storeName: string }) {
  ...
  return <div className="label-page"><div className="page-head print-toolbar"><div><h2>商品標籤與條碼列印</h2><p>已針對精臣 B21 準備常用尺寸；列印視窗中請選擇 B21。</p></div><div className="actions"><Link className="btn" href="/products">返回商品管理</Link><button className="btn primary" onClick={print} disabled={labels.length === 0}>列印 {labels.length} 張標籤</button></div></div>
